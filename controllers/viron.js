...
const show = (req, res) => {
  ...
  const result = {
    ...
    color: 'red',
    theme: 'standard',
    thumbnail: `https://${context.getConfigHost()}/img/icon_terminal.jpeg`, // サービスのアイコン等を指定
    tags: [env, 'viron', 'develop'],
    pages: [].concat(
      // QuickView
      ...
      genSection(constant.VIRON_SECTION_MANAGE, {
        // Manage
        [constant.GROUP_MASTER]: [
          {
            id: 'id_masters',
            name: '割当マスタ',
            components: [
              genTableComponent('割当マスタ', 'get', '/id_masters', 'id', [
                {key: 'id', type: 'integer'},
                {key: 'code', type: 'string'},
                {key: 'username', type: 'string'}
              ], ['code', 'username', 'division'], [
                '/id_masters/download/csv',
              ], ['code', 'username', 'division']),
            ],
          },
          {
            ...
            name: '組織マスタ',
            components: [
              genTableComponent('組織マスタ', 'get', '/divisions', 'id', [
                {key: 'id', type: 'integer'},
                {key: 'division_name', type: 'string'}
              ], ['id', 'divison_name'], [
                '/divisions/download/csv',
              ], ['id', 'division_name']),
            ],
          }
        ],
